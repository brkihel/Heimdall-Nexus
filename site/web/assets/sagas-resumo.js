...
async function load(){try{const response=await fetch('/api/sagas/v1/overview?limit=8',{cache:'no-store'});if(!response.ok)return;const data=await response.json();if(!data.available)return;
  ...
  for(const [title,items] of [['Momentos recentes',events.slice(0,3)],['Feitos recentes',feats.slice(0,3)]]){
    if(!items.length)continue;const section=node('section');section.append(node('h3',title));const list=node('ol');for(const event of items){const item=node('li');item.append(node('strong',(event.name||'Viking')+' '+(labels[event.kind]||'registrou')+(event.target?' '+event.target:'')),node('small',when(event.occurred_at)));list.append(item)}section.append(list);region.append(section)}

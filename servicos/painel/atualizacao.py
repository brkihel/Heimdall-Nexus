...
def _git(source: Path, *args: str, timeout: int = 30, allow_file: bool = False) -> str:
    ...
    try:
        ...
        raise UpdateError('o Git não está instalado; rode: sudo apt install git') from error
    except subprocess.TimeoutExpired as error:
        raise UpdateError('o repositório demorou demais para responder') from error
    if done.returncode != 0:
        detail = (done.stderr or done.stdout).strip().splitlines()[-1:] or ['']
        raise UpdateError('falha no Git: ' + detail[0][:200])
    return done.stdout
...
def set_channel(value: object, state: Path = STATE) -> str:
    if not isinstance(value, str) or not CHANNEL.fullmatch(value):
        raise UpdateError('canal inválido')
...
def _ensure_clone(state: Path, repository: str, allow_file: bool) -> Path:
    if not allow_file and not REPO_URL.fullmatch(repository):
        raise UpdateError('endereço do repositório de atualização inválido')
    source = _paths(state)['source']
    if not (source / '.git').is_dir():
        if source.exists():
            raise UpdateError(f'{source} existe mas não é um repositório Git')
...
def start(commit: object, state: Path = STATE, runner=subprocess.run,
    ...
    if running():
        raise UpdateError('uma atualização já está em andamento')
    try:
    ...
    if done.returncode != 0:
        raise UpdateError('não foi possível iniciar a atualização: ' +

"""Optional OpenRouter chapters from consented Sagas facts.
...
DEFAULT = {'enabled': False, 'model': 'openrouter/free',
           'allow_paid': False, 'daily_limit': 3}
MODEL = re.compile(r'^[A-Za-z0-9_.-]+/[A-Za-z0-9_.:+-]{1,100}$')
KEY = re.compile(r'^sk-or-[A-Za-z0-9_-]{14,250}$')
...
def valid_config(value: object) -> dict:
    if not isinstance(value, dict) or set(value) != set(DEFAULT) or \
            not isinstance(value['enabled'], bool) or \
            not isinstance(value['allow_paid'], bool) or \
            ...
            not value['allow_paid'] and value['model'] != 'openrouter/free' and \
            not value['model'].endswith(':free'):
...
def load_config(state: Path) -> dict:
    try:
        return valid_config(json.loads((state / 'story-settings.json').read_text(encoding='utf-8')))
    except (OSError, ValueError):
        return DEFAULT.copy()


def _visible_rows(db: sqlite3.Connection, world: str, actor: str,
                  mode: str, limit: int = MAX_EVIDENCE) -> list[dict]:
    rows = db.execute('''SELECT e.world, e.actor, e.id, e.kind, e.name,
                      ...
                      (world, actor, actor, mode, mode, mode, limit)).fetchall()
...
def _request(key: str, model: str, scope: str, facts: list[dict]) -> dict:
    system = (
    ...
    body = json.dumps({'model': model, 'stream': False, 'max_completion_tokens': 900,
                       'temperature': 0.7, 'messages': [
                           {'role': 'system', 'content': system},
                           {'role': 'user', 'content': json.dumps(
                               {'scope': scope, 'facts': facts}, ensure_ascii=False)}]},
    ...
    try:
        ...
        if len(payload) > MAX_RESPONSE:
            raise StoryError('resposta grande demais do OpenRouter')
        if response.status != 200:
            raise StoryError(f'OpenRouter recusou a solicitação (HTTP {response.status})')
        ...
        raise StoryError('falha de conexão com OpenRouter') from error
    except (KeyError, IndexError, TypeError, ValueError) as error:
        raise StoryError('resposta inválida do OpenRouter') from error
    finally:
        connection.close()
...
def _evidence(result: dict, rows: list[dict]) -> list[dict]:
...
def generate(state: Path, world: str, actor: str = '') -> dict:
    ...
    try:
        key = (state / 'openrouter.key').read_text(encoding='ascii').strip()
    except (OSError, UnicodeError) as error:
        raise StoryError('configure a chave do OpenRouter') from error
    if not valid_key(key):
        raise StoryError('chave do OpenRouter inválida')
    scope = 'viking' if actor else 'server'
    with sagas.connect(state / 'sagas.sqlite3') as db:
        rows = _visible_rows(db, world, actor, sagas_settings['kill_mode'])
        if not rows:
    ...
    result = _request(key, config['model'], scope, facts)
    title = _clean(result.get('title'), 100)
    prose = _clean(result.get('text'), 2400, 120)
    chosen = _evidence(result, rows)
    with sagas.connect(state / 'sagas.sqlite3') as db:
...
def admin_status(state: Path) -> dict:
    ...
    return {'config': config, 'has_key': (state / 'openrouter.key').is_file(),

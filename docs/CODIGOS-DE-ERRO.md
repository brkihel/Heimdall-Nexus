...
|---|---|---|
| `HN-ATL-001` | O mapa Birds Eye não pôde ser gerado | Linux: Confira o erro no servidor: sudo journalctl -u heimdall-sagas-atlas -n 50 --no-pager. Após corrigir a causa, o Nexus tenta de novo em até 10 minutos.<br>Windows: Confira o erro em C:\ProgramData\HeimdallNexus\logs\heimdall-sagas-jobs. Após corrigir a causa, o Nexus tenta de novo em até 10 minutos. |
| `HN-UPD-001` | Git não instalado | Linux: Na máquina do Nexus, rode: sudo apt install git<br>Windows: Instale o Git para Windows (https://git-scm.com/download/win) para todos os usuários e reinicie o serviço heimdall-executor. |
...
| `HN-UPD-004` | Endereço do repositório de atualização inválido | Linux: HEIMDALL_UPDATE_REPO em /etc/heimdall-nexus/heimdall.env precisa ser um endereço https://….git.<br>Windows: HEIMDALL_UPDATE_REPO em C:\ProgramData\HeimdallNexus\etc\heimdall.env precisa ser um endereço https://….git. |
| `HN-UPD-005` | A cópia local do repositório está danificada | Linux: Apague /var/lib/heimdall-nexus/source (sudo rm -rf) e procure atualizações de novo.<br>Windows: Apague C:\ProgramData\HeimdallNexus\state\source e procure atualizações de novo. |
...
| `HN-UPD-011` | Não foi possível iniciar a atualização | Linux: O systemd recusou a tarefa. Veja: sudo journalctl -u heimdall-executor -n 50<br>Windows: O Windows recusou a tarefa agendada. Veja C:\ProgramData\HeimdallNexus\logs\heimdall-executor. |
...
| `HN-UPD-101` | Falha ao copiar o código novo | Linux: Confira o espaço em disco (df -h /opt). A versão anterior continua nos serviços até o reinício.<br>Windows: Confira o espaço livre no disco de C:\Program Files. A versão anterior continua nos serviços até o reinício. |
...
| `HN-UPD-103` | Falha ao atualizar os serviços das Sagas | Linux: Veja: systemctl status heimdall-sagas-ingest.timer heimdall-sagas-story.timer<br>Windows: Veja C:\ProgramData\HeimdallNexus\logs\heimdall-sagas-jobs e o serviço heimdall-sagas-jobs. |
| `HN-UPD-104` | Falha ao atualizar os ajudantes do site | Linux: Confira as permissões de /var/lib/heimdall-nexus/site. Suas páginas não foram alteradas.<br>Windows: Confira as permissões de C:\ProgramData\HeimdallNexus\state\site. Suas páginas não foram alteradas. |
...
| `HN-UPD-108` | O painel não voltou depois de reiniciar | Linux: Veja: sudo journalctl -u heimdall-panel -n 80. Pelo terminal: cd ~/Heimdall-Nexus && sudo ./deploy/update.sh<br>Windows: Veja C:\ProgramData\HeimdallNexus\logs\heimdall-panel. Pelo PowerShell como administrador, na pasta do repositório: & "$env:ProgramFiles\HeimdallNexus\venv\Scripts\python.exe" deploy\windows\update.py |
...
| `HN-STO-018` | Erro interno ao gerar a história | Linux: Veja: sudo journalctl -u heimdall-sagas-story -n 50<br>Windows: Veja C:\ProgramData\HeimdallNexus\logs\heimdall-sagas-jobs. |
